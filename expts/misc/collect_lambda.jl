# ============================================================================
# collect_lambda.jl — gather every diagnostic from the gate x CER sweep
# ============================================================================
# Invoked by `bash misc/sweep_lambda.sh --collect`; runnable directly as
#
#     julia --project="./../" misc/collect_lambda.jl [results_dir] [--outdir DIR]
#
# WRITES THREE FILES (nothing is aggregated away that cannot be recovered):
#
#   lambda_per_run.csv    one row per run — every column of the results CSV,
...
#                           statistics).
#
#   lambda_per_arm.csv    one row per arm — mean, sd, min, max across seeds
...
#
#   lambda_contrasts.csv  the four contrasts the sweep exists to resolve,
...
const RUN_PATTERN = r"_lam([0-9p]+|nocer)_(ungated|gated)_seed_(\d+)\.csv$"
...
function run_suffix(arm::String, gate::String, seed::Int)::String
    suffix::String = "_lam$(arm)_$(gate)_seed_$(seed).csv"
    return suffix
end
...
function debug_log_summary(logs_dir::String, arm::String, gate::String, seed::Int)::Dict{Symbol, Any}
    ...
    run_tail::String = "_lam$(arm)_$(gate)_seed_$(seed)"
...
function collect_contrasts(per_run::DataFrame)::DataFrame
    # Every lambda against lam0 (isolates the COUPLINGS), plus lam0 against the
    # nocer baseline (isolates the PRIORS). The second is why the nocer arm was
    # added: lam0 still carries the rescaled CER single-qubit priors, so without
    # it an improvement at lambda > 0 cannot be separated from a prior effect.
    labels::Vector{String} = unique(per_run.label)
    gate_suffixes::Vector{String} = unique([g for g in per_run.gate])
    pairs = Tuple{String, String}[]
    for gate in gate_suffixes
        zero_label = "0_$(gate)"
        for label in sort(labels)
            if label == zero_label || !endswith(label, "_$(gate)")
                continue
            end
            push!(pairs, (label, zero_label))
        end
        if "nocer_$(gate)" in labels && zero_label in labels
            push!(pairs, (zero_label, "nocer_$(gate)"))
        end
    end
    collected_rows::Vector{Dict{Symbol, Any}} = Dict{Symbol, Any}[]
    for (a, b) in pairs
...
function main(arguments::Vector{String})::Nothing
    ...
    if nrow(per_run) == 0
        error("no lambda-sweep runs found in $(results_dir) " *
              "(expecting names ending _lam<value>_<gate>_seed_<n>.csv)")
    end
    ...
    CSV.write(joinpath(output_dir, "lambda_per_run.csv"), per_run)
    CSV.write(joinpath(output_dir, "lambda_per_arm.csv"), per_arm)
    CSV.write(joinpath(output_dir, "lambda_contrasts.csv"), contrasts)
    println("\n  wrote $(joinpath(output_dir, "lambda_per_run.csv"))   ($(nrow(per_run)) rows x $(ncol(per_run)) cols)")
    println("  wrote $(joinpath(output_dir, "lambda_per_arm.csv"))   ($(nrow(per_arm)) rows)")
    println("  wrote $(joinpath(output_dir, "lambda_contrasts.csv"))   ($(nrow(contrasts)) rows)")
    return nothing
end
