...
# ============================================================================
# WHY: the correlation term  L_corr = -(1/(N*|C|)) sum_C J_ik sigma_i sigma_k
...
#        - retrain  = true      (each point must train its own model)
#        - run_tag  = "_a4<A>_a3<B>"
...
#
# USAGE (from expts/):
#   bash misc/sweep_correlation_weight.sh                       # generate with defaults
#   bash misc/sweep_correlation_weight.sh --pvals "0.0005 0.002"
#   bash misc/sweep_correlation_weight.sh --alpha4 "0 0.01 0.1 1.0" --alpha3 "0.5 5.0"
# then:
#   sbatch ../data/<codename>/cluster/sweep_<timestamp>.sh
#
...
[ -d "$MODELS_DIR" ]   || { echo "no models dir: $MODELS_DIR" >&2; exit 1; }
[ -f "$BASE_HP_PATH" ] || { echo "no base hyperparams: $BASE_HP_PATH" >&2; exit 1; }
mkdir -p "$CLUSTER_DIR"

TS=$(date +%Y-%m-%d_%H-%M-%S)
COMMANDS="$CLUSTER_DIR/sweep_commands_${TS}.txt"
SLURM="$CLUSTER_DIR/sweep_${TS}.sh"
: > "$COMMANDS"

# Turn a numeric value into a filename-safe token: 0.01 -> 0p01, 1.0 -> 1p0
tag_of() { echo "$1" | tr '.' 'p' | tr -d '+'; }

# --------------------------------------------------- generate hyperparams ---
# Strip the keys we override (and any stale ones), then append our versions.
n_points=0
for a4 in $ALPHA4; do
  for a3 in $ALPHA3; do
...
# ---- injected by sweep_correlation_weight.sh ($TS) ----
# Each point must train its own model (the run_tag keeps them separate).
retrain = true
...
EOF
    ...
    for p in $PVALS; do
      echo "julia --project=\"./../\" neural_bp_experiments.jl --workdir \$WORKDIR_RUNTIME --codename $CODENAME --n_hidden_layers $NLAYERS --hyperparams $hp_name --correlation_strengths_file correlated_weights_p_${p}_s_${SEED}.txt --quiet true --train train_p_${p}_s_${SEED}.txt --test test_p_${p}_s_${SEED}.txt" >> "$COMMANDS"
      n_points=$((n_points + 1))
    done
  done
done
...
#SBATCH --job-name=cer_sweep_$TS
#SBATCH --output=$CLUSTER_DIR/sweep_${TS}.out
#SBATCH --error=$CLUSTER_DIR/sweep_${TS}.err
#SBATCH --ntasks=1
...
# Standalone sweep runner ($n_points commands, $JOBS at a time).
...
# Commands were written with a placeholder so the workdir resolves at runtime.
COMMANDS_LOCAL="\$SLURM_TMPDIR/sweep_commands.txt"
sed "s|\\\$WORKDIR_RUNTIME|\$SLURM_TMPDIR|g" "$COMMANDS" > "\$COMMANDS_LOCAL"

LOCAL_LOGS="\$LOCAL_WORK_DIR/cluster/logs/sweep_$TS"
mkdir -p "\$LOCAL_LOGS"
...
echo "generated $n_points command(s) across $(echo $ALPHA4 | wc -w) alpha4 x $(echo $ALPHA3 | wc -w) alpha3 x $(echo $PVALS | wc -w) p"
echo "  hyperparams -> $MODELS_DIR/hyperparams_sweep_a4*_a3*.toml"
