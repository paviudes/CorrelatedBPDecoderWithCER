...
function parse_hyper_parameters(hyperparams_file::String=""; prefix::String="./../data")::Dict{String, Any}
    ...
    default_hyperparams::Dict{String, Any} = Dict(
        ...
        "syndrome_gate_threshold" => -1.0f0, # tau for the per-sample detached syndrome gate on the AUXILIARY loss terms (certainty / sparsity / correlation), in units of softly broken checks. <= 0 DISABLES the gate, which is the historical behaviour and stays the default. When > 0 (0.5 is the natural value: "open below half a broken check") the aux terms apply per sample ONLY where the residual already nearly clears the syndrome, and layer selection sees base_loss alone. Ungated, the aux terms also act on samples that have NOT cleared, where they compete with the syndrome objective rather than choosing among solutions that already satisfy it. NOTE: this key was previously absent here, so the -1.0 fallback lived only in `get(...)` at src/train.jl:353 — every run to date has silently taken the ungated path. Listing it changes nothing; it makes the default findable.
