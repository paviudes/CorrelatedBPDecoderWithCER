...
function parse_hyper_parameters(hyperparams_file::String=""; prefix::String="./../data")::Dict{String, Any}
    ...
    default_hyperparams::Dict{String, Any} = Dict(
        ...
        "coupling_scale_learnable" => true, # Whether alpha is trained (one scalar shared by every coupling, alongside the message weights). false keeps it at coupling_scale_init: the no-new-parameters Bayesian control. Training learns the unconstrained LOGIT of alpha and the forward pass applies alpha = 1/(1+exp(-theta)), so alpha stays strictly inside (0,1) with no clamping: outside that range alpha would either invert the couplings (J already carries the sign) or claim more coupling than CER measured. The learned alpha is written to the weights JSON, the debug hp log and the results CSV, alongside the logit.
        "coupling_schedule" => "constant", # How alpha varies with the LAYER. "constant" is one alpha everywhere (every earlier run). "step" uses alpha_t = alpha * d(t) with d(t) = 1/(1 + exp((t - T0)/w)): full couplings in the early layers where they cut convergence failures, rolled off around layer T0 over w layers where a constant alpha's late commits land in the wrong coset (see src/soft_constraints.jl). T0 and w are TWO trainable parameters, not one per layer: only ~2% of samples are still active past layer 10 and `warmup_layers` hides the first layers from the loss, so ninety free numbers would fit noise. Requires check_node = "enriched". No automatic filename tag -- put e.g. `_sch12w3L` in `run_tag`.
        "coupling_schedule_layer_init" => 12.0f0, # Initial T0 for the step schedule, in layers: where the couplings are half off. Any real is allowed (below 1: couplings start mostly off; far above n_hidden_layers: they never fade). 12 sits between the layers where the couplings help (2-8) and the median layer of the late coset failures they cause (8-16).
        "coupling_schedule_width_init" => 3.0f0, # Initial w for the step schedule, in layers: the roll-off spans roughly 4w layers. Must exceed 0.5 (the floor below which the step's gradient is unbounded); stored and trained as log(w - 0.5).
        "coupling_schedule_learnable" => true, # Whether T0 and w are trained. false keeps the step fixed at its init: that is how a classical scan over (T0, w) is run through standard_bp_experiments.jl. Ignored (the leaf is frozen) under the constant schedule, where the two parameters are never read.
    ...
    if hyperparams_file != "" && isfile(hyperparams_file_path)
        ...
        float32_keys = ["learning_rate", "max_grad_norm", "weight_decay", "adam_eps", "initial_conditions_scale",
                        ...
                        "coupling_schedule_layer_init", "coupling_schedule_width_init"]
