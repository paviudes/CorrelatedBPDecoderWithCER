...
n_qubits  = 90
n_errors  = 100000
PCM_Z=np.loadtxt("Standard_BP_OSD/90q_BB_p_0.006_q_0.1_std_0.1_data/90q_BB_code/HZ.txt", dtype=int)
Log_Z=np.loadtxt("Standard_BP_OSD/90q_BB_p_0.006_q_0.1_std_0.1_data/90q_BB_code/LZ.txt", dtype=int)
    ...
    for p_value in p_values:
        for q_mean in q_means:
            for sample in range(1, n_samples + 1):
                ...
                filename = f"Standard_BP_OSD/90q_BB_p_0.006_q_0.1_std_0.1_data/testing_data/test_ballistic_p_{p_value}_q_{q_mean}_s_{sample}.txt"
...
np.savetxt(
    "Standard_BP_OSD/90q_BB_p_0.006_q_0.1_std_0.1_data/90q_BB_BP+OSD_failure_rates_OSD_E_order_2.txt",
    results,
    header="p_value  q_mean  sample  logical_failures",
    fmt=["%.3f", "%.2f", "%d", "%d"]
)

print("Results saved to 90q_BB_p_0.006_q_0.1_std_0.1_data/90q_BB_BP+OSD_failure_rates_OSD_E_order_2.txt")